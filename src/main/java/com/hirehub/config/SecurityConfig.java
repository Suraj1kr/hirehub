...
public class SecurityConfig {
 ...
 @Bean UserDetailsService userDetailsService(UserDao users){return email->users.findByEmail(email.trim().toLowerCase(Locale.ROOT)).map(u->User.withUsername(u.getEmail()).password(u.getPasswordHash()).roles(u.getRole().name()).build()).orElseThrow(()->new UsernameNotFoundException("Invalid credentials"));}
 @Bean SecurityFilterChain filterChain(HttpSecurity http)throws Exception {
  http.authorizeHttpRequests(auth->auth
   .requestMatchers("/","/jobs","/jobs/*","/login","/register","/css/**","/error","/api/csrf").permitAll()
   ...
   .requestMatchers("/applications/**","/api/applications/**").hasRole("USER")
